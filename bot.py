...
def calc_support_resistance(highs, lows, closes, lookback=20):
    """كيحدد أقرب support و resistance"""
    if len(highs) < lookback:
        return None, None
    recent_highs = highs[-lookback:]
    recent_lows = lows[-lookback:]
    resistance = round(max(recent_highs), 6)
    support = round(min(recent_lows), 6)
    return support, resistance

def calc_trend_structure(closes, period=10):
    """كيشوف واش السوق uptrend أو downtrend"""
    if len(closes) < period * 2:
        return "غير معروف"
    
    # نقارن آخر قمتين وقاعتين
    mid = len(closes) // 2
    first_half_max = max(closes[:mid])
    second_half_max = max(closes[mid:])
    first_half_min = min(closes[:mid])
    second_half_min = min(closes[mid:])
    ...
    if second_half_max > first_half_max and second_half_min > first_half_min:
        ...
        return "↔️ Sideways (بلا اتجاه واضح)"

def get_debug_report(pair):
    """كيشرح علاش البوت ما دخلش trade لهاد الزوج"""
    debug_lines = []
    
    for tf in TIMEFRAMES:
        result = get_cached_data(pair, tf)
        if not result:
            debug_lines.append(f"\n  <b>{tf}:</b> ❌ ما جاتش البيانات من API")
            continue
        ...
        if rsi is None:
            ...
            rsi_status = f"✅ {rsi} (منطقة BUY)"
        elif rsi > 60:
            rsi_status = f"✅ {rsi} (منطقة SELL)"
        else:
            rsi_status = f"⏸ {rsi} (محايد — خاصو < 40 أو > 60)"
        ...
        if macd is None:
            ...
            macd_status = f"✅ {macd} فوق السيغنال {signal} (momentum صاعد)"
        else:
            macd_status = f"❌ {macd} تحت السيغنال {signal} (momentum هابط)"
        
        # ATR
        atr = calc_atr(highs, lows, closes)
        atr_status = f"✅ {atr}" if atr else "❌ ما حسبش"
        
        # EMA200
        ema200 = calc_ema(closes, 200)
        current_price = closes[-1]
        if ema200 is None:
            ema_status = "❌ بيانات ناقصة (خاصها 200 شمعة)"
        elif current_price > ema200:
            ema_status = f"✅ السعر {current_price} فوق EMA200 {ema200} (Uptrend)"
        else:
            ema_status = f"⚠️ السعر {current_price} تحت EMA200 {ema200} (Downtrend)"
        
        # Trend Structure
        trend = calc_trend_structure(closes)
        
        # Support/Resistance
        support, resistance = calc_support_resistance(highs, lows, closes)
        if support and resistance:
            sr_status = f"✅ Support: {support} | Resistance: {resistance}"
        else:
            sr_status = "❌ ما حسبش"
    ...
    buy_count = list(results.values()).count("BUY")
    sell_count = list(results.values()).count("SELL")
    
    if buy_count >= 2:
        summary = f"✅ {buy_count}/3 متفقين على BUY — إشارة مرسلة!"
    elif sell_count >= 2:
        summary = f"✅ {sell_count}/3 متفقين على SELL — إشارة مرسلة!"
    else:
        summary = f"❌ BUY:{buy_count}/3 | SELL:{sell_count}/3 — ما كاينة حتى اتفاق على 2 من 3"
...
def send_hourly_report(pairs_status):
    ...
    for pair, status in pairs_status.items():
        ...
        # Debug
        debug = get_debug_report(pair)
        if debug:
            msg += f"  📋 <b>Debug:</b>{debug}\n"
